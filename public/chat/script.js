...
function Wo_OpenChatTab(recipient_id, group_id,product_id = 0,page_id = 0,page_user_id = 0,story_id = 0) {
 ...
 if(window.localStorage.getItem('chat_id')){
    ...
    if (listTab.indexOf(recipient_id) == -1) {
        listTab = listTab.concat(",", recipient_id);
       // console.log('ssss'+ listTab);
        window.localStorage.setItem('chat_id' , listTab); 

    }
 }else{
  tabs.push(recipient_id); 
  window.localStorage.setItem('chat_id' , recipient_id); 
 }

...
function Wo_OpenChatTab(recipient_id, group_id,product_id = 0,page_id = 0,page_user_id = 0,story_id = 0) {
 // CheckIfCanUse();
 if(window.localStorage.getItem('chat_id') != null){
    var tabchat = JSON.parse(window.localStorage.getItem('chat_id'));
    if (tabchat.includes(recipient_id) == false) {     
        tabchat.push(recipient_id); 
        window.localStorage.setItem('chat_id' , JSON.stringify(tabchat)); 
    }
 }else{
    const tabchat= [];
    tabchat.push(recipient_id); 

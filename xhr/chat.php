...
if ($f == 'chat') {
                            ...
                            if (isset($recipient['user_uid'])) {
                              ...
                              // $wo['chat']['color']     = 'red';
                               // $wo['chat']['story_id']  = 0;
                                ...
                                //Wo_LoadPage('chat/chat-tab');
                                Wo_RegisterFollow($recipient_id,$wo['user']['user_uid']);
                                Wo_RegisterFollow($wo['user']['user_uid'],$recipient_id);
                                ...
                                /*if (isset($_SESSION['chat_id'])) {
                                    if (strpos($_SESSION['chat_id'], ',') !== false) {
                                        ...
                                        if (count($explode) > 2) {
                                            if (strpos($_SESSION['chat_id'], $recipient['user_uid']) === false) {
                                                $_SESSION['chat_id'] = substr($_SESSION['chat_id'], 0, strrpos($_SESSION['chat_id'], ','));
                                                $_SESSION['chat_id'] .= ',' . $recipient['user_uid'];
                                            }
                                        } else {
                                            $_SESSION['chat_id'] .= ',' . $recipient['user_uid'];
                                        }
                                    } else if (strpos($_SESSION['chat_id'], $recipient['user_uid']) === false) {
                                        $_SESSION['chat_id'] .= ',' . $recipient['user_uid'];
                                    } else {
                                        $_SESSION['chat_id'] = $recipient['user_uid'];
                                    }
    ...
    if ($s == 'load_chat_messages') {
        if (!empty($_GET['recipient_id'])  && $_GET['recipient_id'] != '' && Wo_CheckMainSession($hash_id) === true) {
            $recipient_id = Wo_Secure($_GET['recipient_id']);
            ...
            $wo['chat']['color'] = Wo_GetChatColor($wo['user']['user_uid'], $recipient_id);
        ...
        if ($wo['config']['who_upload'] == 'pro' && $wo['user']['is_pro'] == 0 && !Wo_IsAdmin() && (!empty($_FILES['sendMessageFile']) || !empty($_POST['message-record']))) {
            ...
            if ($wo['user']['message_privacy'] != 2) {
                if (!empty($_POST['user_id']) && Wo_CheckMainSession($hash_id) === true) {
                    ...
                    if (!empty($user_data) && $user_data['message_privacy'] == 1 && Wo_IsFollowing($wo['user']['user_uid'], $_POST['user_id']) === false) {
                        exit();
                    }
                    ...
                    $messages = Wo_RegisterMessage(array(
                        'from_id' => Wo_Secure($wo['user']['user_uid'],0),
                    ...
                    if ($messages > 0) {
                        ...
                        $wo['chat']['color'] = Wo_GetChatColor($wo['user']['user_uid'], $_POST['user_id']);
                        foreach ($messages as $wo['chatMessage']) {
                            $html .= Wo_LoadPage('chat/chat-list');
                        }
                        $file = false;

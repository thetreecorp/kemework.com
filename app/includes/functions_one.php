...
function Wo_UserData($user_id, $password = true) {
    ...
    $data           = array();
    //$user_id        = Wo_Secure($user_id);
    $query_one      = "SELECT * FROM " . T_USERS . " WHERE `user_uid` = '{$user_id}'";
   ...
   // $fetched_data['avatar_org'] = $fetched_data['photo'];
   $fetched_data['avatar'] = $fetched_data['photo'];
  ...
  //  $fetched_data['cover']         = Wo_GetMedia($fetched_data['cover']) . '?cache=' . $fetched_data['last_cover_mod'];
    $fetched_data['id']            = $fetched_data['user_uid'];
...
function Wo_RegisterFollow($following_id = 0, $followers_id = 0) {
    ...
    if (!isset($following_id) or empty($following_id)) {
        return false;
    }
    ...
    foreach ($followers_id as $follower_id) {
        if (!isset($follower_id) or empty($follower_id)) {
            continue;
        }
       /* if (Wo_IsBlocked($following_id)) {
        ...
        if (empty($follower_data['user_uid']) || empty($following_data['user_uid'])) {
            continue;
        }
...
function Wo_GetMessages($data = array(), $limit = 50) {
    ...
    $logged_user_id = Wo_Secure($wo['user']['user_uid']);
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            ...
            $fetched_data['onwer']       = ($fetched_data['messageUser']['user_uid'] == $wo['user']['user_uid']) ? 1 : 0;
...
function GetMessageById($id) {
    ...
    if (mysqli_num_rows($query)) {
        ...
        while ($fetched_data = mysqli_fetch_assoc($query)) {
           ...
           // $fetched_data['text']        = Wo_Markup($fetched_data['text']);
           // $fetched_data['text']        = Wo_Emo($fetched_data['text']);
          //  $fetched_data['onwer']       = ($fetched_data['messageUser']['user_id'] == $wo['user']['user_uid']) ? 1 : 0;
           /* if (!empty($fetched_data['stickers']) && !Wo_IsUrl($fetched_data['stickers'])) {
                $fetched_data['stickers'] = Wo_GetMedia($fetched_data['stickers']);
            }*/
            if ($fetched_data['messageUser']['user_id'] == $wo['user']['user_uid'] && $fetched_data['seen'] == 0) {
                mysqli_query($sqlConnect, " UPDATE " . T_MESSAGES . " SET `seen` = " . time() . " WHERE `id` = " . $fetched_data['id']);
            }
          //  $fetched_data['reaction'] = Wo_GetPostReactionsTypes($fetched_data['id'], 'message');
            $fetched_data['pin']      = 'no';
            $mute                     = $db->where('user_id', $wo['user']['user_uid'])->where('message_id', $fetched_data['id'])->where('pin', 'yes')->getOne(T_MUTE);
            if (!empty($mute)) {
                $fetched_data['pin'] = 'yes';
            }
            $fetched_data['fav']      = 'no';
            $mute                     = $db->where('user_id', $wo['user']['user_uid'])->where('message_id', $fetched_data['id'])->where('fav', 'yes')->getOne(T_MUTE);
            if (!empty($mute)) {
...
function Wo_CountMessages($data = array(), $type = '') {
    ...
    if (empty($data['user_id']) or $data['user_id'] == '') {
        $data['user_id'] = $wo['user']['user_uid'];
    }

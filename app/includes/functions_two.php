...
function Wo_GetChatUsers($type) {
    ...
    $user_id    = Wo_Secure($wo["user"]["user_uid"]);
    $query_text = "SELECT `user_uid` FROM " . T_USERS . " WHERE `user_uid` IN (SELECT `following_id` FROM " . T_FOLLOWERS . " WHERE `follower_id` = '{$user_id}' AND `following_id` <> '{$user_id}' AND `user_uid` NOT IN (SELECT `blocked` FROM " . T_BLOCKS . " WHERE `blocker` = '{$user_id}') AND `user_uid` NOT IN (SELECT `blocker` FROM " . T_BLOCKS . " WHERE `blocked` = '{$user_id}'))";
    if ($type == "online") {
        $query_text .= " AND `lastseen` > {$time}";
    } elseif ($type == "offline") {
        $query_text .= " AND `lastseen` < {$time}";
    }
    $query_text .= " ORDER BY `lastseen` DESC";
    if ($type == "offline") {
        $query_text .= " LIMIT 6";
    }
    ...
    if (mysqli_num_rows($query)) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
            $data[] = Wo_UserData($fetched_data["user_uid"]);
        }
    }
    return $data;
}
...
function Wo_UpdateStatus($status = "online") {
    ...
    $query = mysqli_query($sqlConnect, "UPDATE " . T_USERS . " SET `status` = '{$finel_status}' WHERE `user_uid` = '{$user_id}'");
    if ($query) {
       /* if ($wo["config"]["cacheSystem"] == 1) {
            $cache->delete(md5($wo["user"]["user_uid"]) . "_U_Data.tmp");
        }*/
        return $finel_status;
    }
}
...
function Wo_CreateMainSession() {
    ...
    if (!empty($_COOKIE["main_hash_id"])) {
        $_COOKIE["main_hash_id"] = $_COOKIE["main_hash_id"];
        return $_COOKIE["main_hash_id"];
    }
   // $_SESSION["main_hash_id"] = $hash;
    setcookie("main_hash_id", $hash, time() + 10 * 365 * 24 * 60 * 60);
    return $hash;
}
function Wo_CheckMainSession($hash = "") {
    if (!isset($_COOKIE["main_hash_id"]) || empty($_COOKIE["main_hash_id"])) {
        return false;
    }
    if (empty($hash)) {
        return false;
    }
    if ($hash == $_COOKIE["main_hash_id"]) {
        return true;
    }
    return false;
}

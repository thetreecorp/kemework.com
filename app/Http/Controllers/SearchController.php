...
class SearchController extends Controller
{
    ...
    public function test(){
        $user_ids = User::where('user_type', 'freelancer')->orWhere('user_type','agent')->pluck('id');
    ...
    public function index(Request $request){
        if($request->type == 'freelancer'){
            ...
            if($request->keyword != null){
                $user_ids = User::where('user_type', 'freelancer')->orWhere('user_type','agent')->where('name', 'like', '%'.$keyword.'%')->pluck('id');
                $user_with_pkg_ids = UserPackage::where('package_invalid_at', '!=', null)
                        ->where('package_invalid_at', '>', Carbon::now()->format('Y-m-d'))
                        ->whereIn('user_id', $user_ids)
                        ->pluck('user_id');
            ...
            $freelancers = UserProfile::query();
            if($request->keyword != null){
                $user_ids = User::where('user_type', 'agent')->where('name', 'like', '%'.$keyword.'%')->pluck('id');
                $user_with_pkg_ids = UserPackage::where('package_invalid_at', '!=', null)
                        ->where('package_invalid_at', '>', Carbon::now()->format('Y-m-d'))
                        ->whereIn('user_id', $user_ids)
                        ->pluck('user_id');

...
if ($wo['chatMessage']['from_id'] != $wo['user']['user_uid']) {
...
<div class="messages-wrapper <?php echo ($wo['chatMessage']['onwer'] == 1) ? 'pull-right' : 'pull-left';?> messages-text" id="messageId_<?php echo $wo['chatMessage']['id'] ?>" data-message-id="<?php echo $wo['chatMessage']['id'] ?>"  display="<?php echo ($wo['config']['node_socket_flow'] == "1") ? "hidden" : "visible" ?>">
	...
	<div class="message  <?php echo ($wo['chatMessage']['onwer'] == 1) ? 'outgoing pull-right' : 'incoming pull-left';?>" onclick="Wo_ShowMessageOptions(<?php echo $wo['chatMessage']['id'] ?>);">
		<?php if (!empty($wo['chatMessage']['text'])): ?>
			<div class="message-text" id="message_text_reply_<?php echo $wo['chatMessage']['id'] ?>" dir="auto">
		...
		<?php if (!empty($wo['chatMessage']['story'])) {
			if (!empty($wo['chatMessage']['story']['thumbnail'])) {
				...
				$u = Wo_UserData($wo['chatMessage']['story']['user_uid']);
				if (!empty($u)) {
	...
	<div class="clear"></div>
	<?php if($wo['chatMessage']['messageUser']['user_uid'] == $wo['user']['user_uid']) { ?>

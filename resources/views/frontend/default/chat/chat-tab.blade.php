<div class="chat-wrapper chat_<?php echo $wo['chat']['recipient']['user_uid'];?>" id="chat_<?php echo $wo['chat']['recipient']['user_uid'];?>">
	<div class="online-toggle-hdr pointer" onclick="toggleChat(this, event);" data-chat-id="<?php echo $wo['chat']['recipient']['user_uid'];?>">
		<div class="wow_chat_hdr_usr">
			<div class="avatar">
				<a href="<?php echo $wo['chat']['recipient']['url'];?>" data-ajax="?link1=timeline&u=<?php echo $wo['chat']['recipient']['name'];?>">
			...
			<div>
				<a href="<?php echo $wo['chat']['recipient']['url'];?>" data-ajax="?link1=timeline&u=<?php echo $wo['chat']['recipient']['name'];?>"> 
					...
					<p class="chat-tab-status <?php echo (Wo_IsOnline($wo['chat']['recipient']['user_uid']) === true) ? 'active' : ''; ?>">
						<svg xmlns="http://www.w3.org/2000/svg" width="9" height="9" viewBox="0 0 24 24"><path fill="currentColor" d="M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12A10,10 0 0,0 12,2Z"></path></svg> online
					</p>
				</a>
			</div>
		</div>
		...
		<div class="close-chat">
			...
			<?php if ($wo['chat']['recipient']['lastseen'] > (time() - 60)) { ?>
				<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" onclick="Wo_GenerateVoiceCall(<?php echo $wo['user']['user_uid'];?>, <?php echo $wo['chat']['recipient']['user_uid'];?>)"><path fill="currentColor" d="M6.62,10.79C8.06,13.62 10.38,15.94 13.21,17.38L15.41,15.18C15.69,14.9 16.08,14.82 16.43,14.93C17.55,15.3 18.75,15.5 20,15.5A1,1 0 0,1 21,16.5V20A1,1 0 0,1 20,21A17,17 0 0,1 3,4A1,1 0 0,1 4,3H7.5A1,1 0 0,1 8.5,4C8.5,5.25 8.7,6.45 9.07,7.57C9.18,7.92 9.1,8.31 8.82,8.59L6.62,10.79Z" /></svg>
			<?php } else { ?>
			...
			<?php if ($wo['config']['video_chat'] == 1 && ($wo['config']['twilio_video_chat'] == 1 || $wo['config']['agora_chat_video'] == 1)) {
			 ...
			 ?>
			<?php if ($wo['chat']['recipient']['lastseen'] > (time() - 60)) { ?>
				<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" onclick="Wo_GenerateVideoCall('<?php echo $wo['user']['user_uid'];?>', '<?php echo $wo['chat']['recipient']['user_uid'];?>')"><path fill="currentColor" d="M17,10.5V7A1,1 0 0,0 16,6H4A1,1 0 0,0 3,7V17A1,1 0 0,0 4,18H16A1,1 0 0,0 17,17V13.5L21,17.5V6.5L17,10.5Z" /></svg>
			<?php } else { ?>
			...
			<?php } } } ?>
			<a class="white" href="<?php echo Wo_SeoLink("index.php?link1=messages&user=" . $wo['chat']['recipient']['user_uid']);?>" target="_blank" onclick="Wo_CloseChat('<?php echo $wo['chat']['recipient']['user_uid'];?>');">
			...
			<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="width: 24px; height: 24px;"onclick="Wo_CloseChat('<?php echo $wo['chat']['recipient']['user_uid'];?>');"><path fill="currentColor" d="M19,6.41L17.59,5L12,10.59L6.41,5L5,6.41L10.59,12L5,17.59L6.41,19L12,13.41L17.59,19L19,17.59L13.41,12L19,6.41Z" /></svg>
		</div>
	</div>

	<div class="chat-tab-container chat-tab-container-<?php echo $wo['chat']['recipient']['user_uid'];?> chat-tab-box-<?php echo $wo['chat']['recipient']['user_uid'];?>">
		<div class="chat-messages-wrapper chat-messages-wrapper-<?php echo $wo['chat']['recipient']['user_uid'];?>">
			<div class="chat-messages">
				<div class="chat-user-desc">
					<img src="<?php echo $wo['chat']['recipient']['avatar'];?>" alt="User avatar">
					<div class="text">
						<p class="head-text">
							<?php $is_following = Wo_IsFollowing($wo['chat']['recipient']['user_uid'], $wo['user']['user_uid']);?>
		...
		<div class="chat-textarea">
			<form action="#" method="post" class="chat-sending-form-<?php echo $wo['chat']['recipient']['user_uid'];?>">
				<?php 
					$story_id = 0;
				...
				<?php } ?>
				<input type="hidden" name="reply_id" class="message_reply_id_<?php echo $wo['chat']['recipient']['user_uid'];?>" readonly>
				<div class="chat_reply_text message_reply_text_<?php echo $wo['chat']['recipient']['user_uid'];?>" style="display: none;">
					<span></span>
					<svg onclick="Wo_ClearReplyChatMessage('<?php echo $wo['chat']['recipient']['user_uid'];?>')" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="pointer"><path fill="currentColor" d="M12,2C17.53,2 22,6.47 22,12C22,17.53 17.53,22 12,22C6.47,22 2,17.53 2,12C2,6.47 6.47,2 12,2M15.59,7L12,10.59L8.41,7L7,8.41L10.59,12L7,15.59L8.41,17L12,13.41L15.59,17L17,15.59L13.41,12L17,8.41L15.59,7Z" /></svg>
				</div>
				<textarea name="textSendMessage" id="sendMessage" class="form-control" cols="10" rows="5" placeholder="type_message" onkeydown="Wo_SubmitChatForm(event, '<?php echo $wo['chat']['recipient']['user_uid'];?>');" onfocus="Wo_SubmitChatForm(event, '<?php echo $wo['chat']['recipient']['user_uid'];?>');" dir="auto" ></textarea>
				<div class="chat-btns-w">
					...
					<span class="btn btn-file chat_optns" id="progressIcon">
						<?php if($wo['config']['fileSharing'] == 1) { ?>
							...
							<input type="file" id="sendMessasgeFile" name="sendMessageFile" class="pointer" onchange="Wo_ShareChatFile('<?php echo $wo['chat']['recipient']['user_uid'];?>');" />
						<?php } else { ?>
							...
							<input type="file" id="sendMessasgeFile" name="sendMessageFile" onchange="Wo_ShareChatFile('<?php echo $wo['chat']['recipient']['user_uid'];?>');" accept="image/x-png, image/gif, image/jpeg"  />
						<?php } ?>
					</span>
					<?php if ($wo['config']['audio_upload'] == 1) { ?>
						<span class="btn btn-file record-chat-audio chat_optns" data-record="0" data-chat-tab="<?php echo $wo['chat']['recipient']['user_uid'];?>">
							<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1"  width="24" height="24" viewBox="0 0 24 24" class="select-color"><path fill="<?php echo $wo['chat']['color']; ?>" d="M12,2A3,3 0 0,1 15,5V11A3,3 0 0,1 12,14A3,3 0 0,1 9,11V5A3,3 0 0,1 12,2M19,11C19,14.53 16.39,17.44 13,17.93V21H11V17.93C7.61,17.44 5,14.53 5,11H7A5,5 0 0,0 12,16A5,5 0 0,0 17,11H19Z" /></svg>
						</span>
					<?php } ?>
					<span data-chat-rtime="<?php echo $wo['chat']['recipient']['user_uid'];?>" class="record_timei hidden">00:00</span>
						...
						<span class="dropup chat_optns" id="chat-gifs">
							...
							<ul class="dropdown-menu drop-up" role="menu" onclick="event.stopPropagation()">
								<div class="w100" id="chat-box-stickers">
									<input type="text" class="form-control" placeholder="search GIFs" onkeyup="Wo_GetChatStickers('<?php echo $wo['chat']['recipient']['user_uid'];?>',this.value)">
									<div id="chat-box-stickers-cont-<?php echo $wo['chat']['recipient']['user_uid'];?>" style="height: 300px;overflow: auto;" onscroll="GifScrolledCH(this)" GId="<?php echo $wo['chat']['recipient']['user_uid'];?>" GWord="" GOffset=""></div>
								</div>
							</ul>
						</span>
					<?php endif; ?>
										...
										<?php 
											...
											if( count( $stickers_system ) > 0 ){
												foreach ($stickers_system as $wo['stickerlist']) {
													echo '<img alt="gif" src="'. Wo_GetMedia( $wo['stickerlist']['media_file'] ).'" data-gif="'.Wo_GetMedia( $wo['stickerlist']['media_file'] ).'" onclick="Wo_ChatSticker('. $wo['chat']['recipient']['user_uid'] .',this);" autoplay loop>';
												}
											} else {
						...
						<ul class="dropdown-menu drop-up wow_choose_chat_clrs" role="menu">
							<div class="chat-colors-cont text-center">
								<?php foreach ($wo['colors'] as $key => $color) {?>
									<a class="chat-color" data-chat-color="<?php echo $color; ?>" data-recipient-u-id="<?php echo $wo['chat']['recipient']['user_uid'];?>" style="color: <?php echo $color; ?>;">
					...
					<div>
						<span class="dropup chat_optns">
							<span class="emo-btn-<?php echo $wo['chat']['recipient']['user_uid'];?> dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false" onclick="load_ajax_chat_emojii('<?php echo $wo['chat']['recipient']['user_uid'];?>','<?php echo $wo['config']['theme_url'];?>/emoji/');">
							...
							<ul class="emo-container dropdown-menu emo-container-<?php echo $wo['chat']['recipient']['user_uid'];?>" role="menu">
				...
				<input type="hidden" id="user-id" class="chat-user-id" name="user_id" value="<?php echo $wo['chat']['recipient']['user_uid'];?>" />
		...
		<style>
			#chat_<?php echo $wo['chat']['recipient']['user_uid']; ?> .close-chat a, #chat_<?php echo $wo['chat']['recipient']['user_uid']; ?> .close-chat svg {color:  <?php echo $wo['chat']['color']; ?>}
			</style>
			...
			$(function() {
				...
				$('.emo-btn-<?php echo $wo['chat']['recipient']['user_uid'];?>').click(function () {
					$('.emo-container-<?php echo $wo['chat']['recipient']['user_uid'];?>').toggle();
				});
			
			  var chat_messages_wrapper = $('.chat-messages-wrapper-<?php echo $wo['chat']['recipient']['user_uid'];?>');
				console.log("Private message")
			  <?php if ($wo['config']['node_socket_flow'] == "1") { ?>
				var color = $('.chat-sending-form-<?php echo $wo['chat']['recipient']['user_uid'];?> #color').val();
				$('.chat-messages-wrapper-<?php echo $wo['chat']['recipient']['user_uid'];?>').scroll(
				()=>{
				  var st = $('.chat-messages-wrapper-<?php echo $wo['chat']['recipient']['user_uid'];?>').scrollTop();  
				  if(st==0) {
					socket.emit("loadmore", {from_id: _getCookie("user_id"), color: color, to_id: '<?php echo $wo['chat']['recipient']['user_uid'];?>', before_message_id: $(".chat-messages").find(".messages-wrapper").attr("id").substr("messageId_".length)}, (data)=>{
					  var chat_messages_wrapper = $('.chat-messages-wrapper-<?php echo $wo['chat']['recipient']['user_uid'];?>');
					  chat_messages_wrapper.find(".chat-messages").prepend(data.html); 
					})
				  }
				})
				$(".chat-sending-form-<?php echo $wo['chat']['recipient']['user_uid'];?>").find(".chat-color").click(function() {
				  let color = $(this).attr("data-chat-color")
				  socket.emit("color-change", { color: color, from_id: _getCookie("user_id"), id: '<?php echo $wo['chat']['recipient']['user_uid'];?>' })
				})
					$('form.chat-sending-form-<?php echo $wo['chat']['recipient']['user_uid'];?>').submit(()=>{
				  if (chat_messages_wrapper.find('.chat-user-desc').length == 1) {
					  chat_messages_wrapper.find('.chat-user-desc').hide();
				  }
				  var text_message = escapeHTML($('.chat-sending-form-<?php echo $wo['chat']['recipient']['user_uid'];?> #sendMessage').val());
				  $('.chat-sending-form-<?php echo $wo['chat']['recipient']['user_uid'];?>').attr('disabled', true);
				  var color = $('.chat-sending-form-<?php echo $wo['chat']['recipient']['user_uid'];?> #color').val();
				  ...
				  <?php } ?>
				  if (!text_message && $('form.chat-sending-form-<?php echo $wo['chat']['recipient']['user_uid'];?>').find('input.message-record').val() == '' && $('form.chat-sending-form-<?php echo $wo['chat']['recipient']['user_uid'];?>').find('#chatSticker').val() == '') {
					$('form.chat-sending-form-<?php echo $wo['chat']['recipient']['user_uid'];?>').find('#sendMessasgeFile').val('');
					return false;
				  }
				  $('body').attr('sending-<?php echo $wo['chat']['recipient']['user_uid'];?>', true);
				  if (text_message && $('form.chat-sending-form-<?php echo $wo['chat']['recipient']['user_uid'];?> #sendMessasgeFile').val() == '') {
				  ...
				  // }, 10);
						$('form.chat-sending-form-<?php echo $wo['chat']['recipient']['user_uid'];?>').clearForm();
				 
				 if(text_message!="" || text_message.trim() != ""){
					 message_reply_id = $('.message_reply_id_<?php echo $wo['chat']['recipient']['user_uid'];?>').val();
					 story_id = $('#story_id').val();
				  socket.emit("private_message", {
					  to_id: '<?php echo $wo['chat']['recipient']['user_uid'];?>', 
					  from_id: _getCookie("user_id"),
					  username: '<?php echo $wo['user']['name']; ?>',
					  msg: text_message,
					  color: color,
					  isSticker: false,
					...
					if (data.status == 200) {
						Wo_ClearReplyChatMessage('<?php echo $wo['chat']['recipient']['user_uid'];?>');
						$('#story_id').val('0');
						...
						if (message_reply_id != 0) {
						  ...
						  $('.message_reply_id_<?php echo $wo['chat']['recipient']['user_uid'];?>').val(0);
						  $('.message_reply_text_<?php echo $wo['chat']['recipient']['user_uid'];?>').fadeOut(50);
						$('form.chat-sending-form-<?php echo $wo['chat']['recipient']['user_uid'];?>').find('input.message-record').val('');   
						$('form.chat-sending-form-<?php echo $wo['chat']['recipient']['user_uid'];?>').find('input.media-name').val('');
						$("#chatSticker").val('');
						...
						if (data.invalid_file == 1) {
						  ...
						  $('form.chat-sending-form-<?php echo $wo['chat']['recipient']['user_uid'];?>').find('.ball-pulse').fadeOut(100);
						}
						...
						$('body').attr('sending-<?php echo $wo['chat']['recipient']['user_uid'];?>', false);
						if (data.file == true) {
						  $('form.chat-sending-form-<?php echo $wo['chat']['recipient']['user_uid'];?>').find('.ball-pulse').fadeOut(100);
						}
				  ...
				  $('form.chat-sending-form-<?php echo $wo['chat']['recipient']['user_uid'];?>').ajaxSubmit({
				...
				uploadProgress: function () {
					$('form.chat-sending-form-<?php echo $wo['chat']['recipient']['user_uid'];?>').find('.ball-pulse').fadeIn(100);
				},
				success: function(data) {
					Wo_ClearReplyChatMessage('<?php echo $wo['chat']['recipient']['user_uid'];?>');
					if (data.status == 200) {
				...
				?>
				$('form.chat-sending-form-<?php echo $wo['chat']['recipient']['user_uid'];?>').ajaxForm({
					...
					beforeSend: function() {
						...
						<?php } else { ?>
						  var html_message = '<div class="sended_message"><div class="messages-wrapper pull-right messages-text" id="messageId_" data-message-id=""><div class="message outgoing pull-right"><p class="message-text" dir="auto">' + text_message + '</p><div class="clear"></div><div class="message-media"></div></div><div class="clear"></div><div class="message-seen text-right message-details"></div><div class="clear"></div><div class="message-typing message-details"></div></div><div class="clear"></div></div>';
						<?php } ?>
						if (!text_message && $('form.chat-sending-form-<?php echo $wo['chat']['recipient']['user_uid'];?>').find('input.message-record').val() == '' && $('form.chat-sending-form-<?php echo $wo['chat']['recipient']['user_id'];?>').find('#chatSticker').val() == '') {
						  $('form.chat-sending-form-<?php echo $wo['chat']['recipient']['user_uid'];?>').find('#sendMessasgeFile').val('');
						  return false;
						}
						$('body').attr('sending-<?php echo $wo['chat']['recipient']['user_uid'];?>', true);
			...
			var sent_seen_id = 0;
			$(document).on('click focus', '.chat-tab-container-<?php echo $wo['chat']['recipient']['user_uid'];?>', function(event) {
				var chat_container = $('.chat-tab').find('.chat_main_<?php echo $wo['chat']['recipient']['user_uid'];?>');
				$('#chat_<?php echo $wo['chat']['recipient']['user_uid'];?>').find('.online-toggle-hdr').attr('style', '').removeClass('white_online');
				var last_id = chat_container.find('.messages-text:last').attr('data-message-id');
				if (sent_seen_id != last_id) {
					sent_seen_id = last_id;
					<?php if ($wo['config']['node_socket_flow'] == "1"){ ?>
						socket.emit("seen_messages", {user_id: _getCookie("user_id"), recipient_id: '<?php echo $wo['chat']['recipient']['user_uid'];?>',message_id: last_id}, (data)=>{})
					<?php }else{ ?>
						$.post( Wo_Ajax_Requests_File() + '?f=chat&s=seen&hash=' + $('.main_session').val(), {recipient_id: '<?php echo $wo['chat']['recipient']['user_uid'];?>'}, function(data, textStatus, xhr) {});
					<?php } ?>
				}
			});
			</script>
